...
/******************************* 8x8 MATRIX OBJECT - AliExpress version */

AliExpress_8x8matrix::AliExpress_8x8matrix(void) : Adafruit_GFX(8, 8) {}

void AliExpress_8x8matrix::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((y < 0) || (y >= 8))
    return;
  if ((x < 0) || (x >= 8))
    return;
